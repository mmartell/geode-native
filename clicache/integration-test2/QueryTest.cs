    ...
    public void PdxSerializableQueryHaveCorrectValues()
    {
      using (var cluster = new Cluster(output, CreateTestCaseDirectoryName(), 1, 1))
      {
        ...
        try
        {
          ...
          var order1 = new QueryOrder(1, "product x", 10);
          var order2 = new QueryOrder(2, "product y", 20);
          var order3 = new QueryOrder(3, "product z", 30);
          ...
          order2.Quantity = UInt16.MaxValue/2 - 3;
          region.Put("order2", order2);

          order3.Quantity = UInt16.MaxValue/2 - 4;
          region.Put("order3", order3);

          var res = region.Get("order3");

          var results = region.Query<QueryOrder>("SELECT * FROM /cqTestRegion WHERE quantity > 50");
          Assert.Equal(results.Size, 2UL);
          Assert.Equal(results[0].Name, "product y");
          Assert.Equal(results[1].Name, "product z");
