  ...
  public class Program
  {
    ...
    public static void Main(string[] args)
    {
      ...
      for (int i=0; i<MAXPHOTOKEYS; i++)
      {
        ...
        Console.WriteLine(
          "Inserting " + numPhotos + " photos for key: " + keys[i].ToString());
      ...
      for (int k = 0; k < MAXPHOTOKEYS; k++)
      {
        ...
        for (int p = 0; p < value.photosMeta.Count; p++)
        {
          ...
          if (meta.fullResId != photoId)
            Console.WriteLine(
              "      ERROR: Expected fullResId = " +
              photoId + " but actual = " + meta.fullResId);
          ...
          for (int i=0; i<PhotoMetaData.THUMB_HEIGHT; i++)
          {
            ...
            {
              if (meta.thumbnailImage.GetPixel(i,j) != 
                values[k].photosMeta[p].thumbnailImage.GetPixel(i,j))
              {
                Console.WriteLine(
                  "      ERROR: Unexpected thumb for photoId = " + photoId);
                thumbValid = false;
                break;
              }
            }
    ...
    public static IRegion<PhotosKey, PhotosValue> CreateRegion()
    {
      ...
      cache.TypeRegistry.RegisterPdxType(PhotosKey.CreateDeserializable);
      cache.TypeRegistry.RegisterPdxType(PhotosValue.CreateDeserializable);
      cache.TypeRegistry.RegisterPdxType(PhotoMetaData.CreateDeserializable);
      ...
      var regionFactory = cache.CreateRegionFactory(RegionShortcut.PROXY)
        .SetPoolName("pool");
      IRegion<PhotosKey, PhotosValue> photosMetaData = regionFactory
        .Create<PhotosKey, PhotosValue>("photosMetaData");
    ...
    public static List<String> ChoosePeople()
    {
      ...
      // Sort the chosen. We only care who is chosen, not the order
      // they're chosen.
    ...
    public static Bitmap ChooseThumb()
    {
      ...
      for (int j=0; j<thumbHeight; j++)
      {
        ...
        {
          thumb.SetPixel(i, j, Color.FromArgb(
